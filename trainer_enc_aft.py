...
class EmotionDataLoaderBar(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        src = src + [tag] + [self.pad_int] * (self.pad_len - len(src) - 1)
...
if __name__ == '__main__':
    ...
    del df, X, y, tag
    training_set = EmotionDataLoaderBar(X_train, y_train, tag_train, pad_len, word2id)
    train_loader = DataLoader(training_set, batch_size=batch_size)

    test_set = EmotionDataLoaderBar(X_dev, y_dev, tag_dev, pad_len, word2id)
    ...
    # model_path = 'checkpoint/new_simple_bar_epoch_9.model'
    ...
    for epoch in range(100):
        ...
        for i, (src, trg) in tqdm(enumerate(train_loader), total=int(len(training_set) / batch_size)):
            # print('i=%d: ' % (i))
            decoder_logit = model(Variable(src).to(device), Variable(trg).to(device))
            optimizer.zero_grad()
        ...
        torch.save(
            ...
            open(os.path.join(
                'checkpoint',
                'new_simple_bar' + '_epoch_%d' % (epoch) + '.model'), 'wb'
            )
        )
