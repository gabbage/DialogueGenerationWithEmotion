...
class Seq2SeqAttentionSharedEmbedding(nn.Module):
    ...
    def __init__(
        ...
        self.emo_transform = nn.Embedding(NUM_EMO + 1, trg_hidden_dim*trg_hidden_dim)
    ...
    def forward(self, input_src, input_trg, tag, trg_mask=None, ctx_mask=None):
        ...
        #     trg_h.size()[2]
        # )
        emo_transformed = torch.bmm(trg_h, self.emo_transform(tag).view(-1, self.trg_hidden_dim, self.trg_hidden_dim))

        decoder_logit = self.decoder2vocab(emo_transformed.view(-1, self.trg_hidden_dim))
