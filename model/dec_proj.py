...
class Seq2SeqAttentionSharedEmbedding(nn.Module):
    ...
    def forward(self, input_src, input_trg, tag, trg_mask=None, ctx_mask=None):
        ...
        #     trg_h.size()[2]
        # )
        decoder_logit = torch.bmm(trg_h, self.decoder2vocab(tag).view(-1, self.trg_hidden_dim, self.vocab_size))
        # decoder_logit = self.decoder2vocab(tag).view(trg_h_reshape)
