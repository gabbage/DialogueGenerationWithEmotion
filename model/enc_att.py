...
class SoftDotAttention(nn.Module):
    ...
    def __init__(self, dim):
        ...
        self.linear_in = nn.Embedding(NUM_EMO + 1, dim*dim)
        self.sm = nn.Softmax()
        self.linear_out = nn.Embedding(NUM_EMO + 1, dim * 2 * dim)
        self.tanh = nn.Tanh()
        self.mask = None

    def forward(self, input, context, tag):
        ...
        target = torch.bmm(input.unsqueeze(1), self.linear_in(tag).view(-1, dim, dim)).transpose_(1, 2)
        ...
        h_tilde = self.tanh(torch.bmm(h_tilde.unsqueeze(1), self.linear_out(tag).view(-1, dim * 2, dim)).squeeze(1))
...
class LSTMAttentionDot(nn.Module):
    ...
    def forward(self, input, hidden, ctx, tag, ctx_mask=None):
        """Propogate input through the network."""
        def recurrence(input, hidden):
            ...
            h_tilde, alpha = self.attention_layer(hy, ctx.transpose(0, 1), tag)
